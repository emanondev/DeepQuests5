...
public abstract class AQuestManager<T extends User<T>> implements QuestManager<T> {
	...
	public AQuestManager(String name, CorePlugin plugin) {
		...
		this.bossBarManager = new ABossBarManager<T>(this);
		this.taskProvider = new ATaskProvider<T>(this);
		this.requireProvider = new ARequireProvider<T>(this);
		this.rewardProvider = new ARewardProvider<T>(this);
		...
		getTaskProvider().registerType(new BreakBlockTaskType<T>(this));
		getTaskProvider().registerType(new BreedMobTaskType<T>(this));
		getTaskProvider().registerType(new FishingTaskType<T>(this));
		getTaskProvider().registerType(new InteractAtTaskType<T>(this));
		getTaskProvider().registerType(new KillMobTaskType<T>(this));
		getTaskProvider().registerType(new PlaceBlockTaskType<T>(this));
		getTaskProvider().registerType(new ShearSheepTaskType<T>(this));
		getTaskProvider().registerType(new TameMobTaskType<T>(this));
		getRequireProvider().registerType(new CompletedMissionRequireType<T>(this));
		getRequireProvider().registerType(new CompletedQuestRequireType<T>(this));
		getRequireProvider().registerType(new FailedMissionRequireType<T>(this));
		getRequireProvider().registerType(new FailedQuestRequireType<T>(this));
		getRequireProvider().registerType(new MissionsPointsRequireType<T>(this));
		getRequireProvider().registerType(new QuestsPointsRequireType<T>(this));
		getRequireProvider().registerType(new CurrentMissionStateRequireType<T>(this));
		getRequireProvider().registerType(new CurrentQuestStateRequireType<T>(this));
		getRequireProvider().registerType(new DayOfWeekRequireType<T>(this));
		getRewardProvider().registerType(new CompleteMissionRewardType<T>(this));
		getRewardProvider().registerType(new CompleteQuestRewardType<T>(this));
		getRewardProvider().registerType(new FailMissionRewardType<T>(this));
		getRewardProvider().registerType(new FailQuestRewardType<T>(this));
		getRewardProvider().registerType(new MissionsPointsRewardType<T>(this));
		getRewardProvider().registerType(new QuestsPointsRewardType<T>(this));
		getRewardProvider().registerType(new StartMissionRewardType<T>(this));
		getRewardProvider().registerType(new SoundRewardType<T>(this));
		if (Hooks.isCitizenEnabled()) {
			getTaskProvider().registerType(new NPCDeliverTaskType<T>(this));
			getTaskProvider().registerType(new NPCTalkTaskType<T>(this));
			getTaskProvider().registerType(new NPCKillTaskType<T>(this));
		}
		if (Hooks.isMythicMobsEnabled()) {
			getTaskProvider().registerType(new KillMythicMobTaskType<T>(this));
		}
		if (Hooks.isRegionAPIEnabled()) {
			getTaskProvider().registerType(new EnterRegionTaskType<T>(this));
			getTaskProvider().registerType(new EnterRegionWithItemTaskType<T>(this));
			getTaskProvider().registerType(new LeaveRegionTaskType<T>(this));
		}

		if (Hooks.isRegionAPIEnabled()) {
			getRequireProvider().registerType(new HaveQuestItemRequireType<T>(this));
			getRewardProvider().registerType(new GetQuestItemRewardType<T>(this));
		}
		if (Hooks.isItemEditEnable()) {
			if (Hooks.isCitizenEnabled())
				getTaskProvider().registerType(new DeliverQuestItemTaskType<T>(this));
			getTaskProvider().registerType(new ObtainQuestItemTaskType<T>(this));
			Quests.get().registerCommand(new QuestBagCommand<T>(this));
		}
	}
		...
		try {
			...
			for (int questId : ids)
				try {
					...
					/*HashSet<Integer> missionsIds = new HashSet<>();
					for (String missionId : questsDB.getKeys(PATH_QUESTS+"."+questId+"."+SUB_PATH_MISSIONS)) {
						try {
							missionsIds.add(Integer.valueOf(missionId));
						} catch (Exception e) {
							e.printStackTrace();
						}
					}*/
					for (int missionId : questsDB.getIntegerList(PATH_QUESTS+"."+questId+"."+SUB_PATH_MISSIONS))
						try {
							...
							/*HashSet<Integer> tasksIds = new HashSet<>();
							for (String taskId : missionsDB.getKeys(PATH_MISSIONS+"."+missionId+"."+SUB_PATH_TASKS))
								try {
...
*/
							for (int taskId : missionsDB.getIntegerList(PATH_MISSIONS+"."+missionId+"."+SUB_PATH_TASKS))
								try {
									...
									/*for (String rewId : tasksDB
											.getKeys(PATH_TASKS + "." + taskId + "." + SUB_PATH_PROGRESS_REWARDS))*/
	...
	public SortableButton getEditorButton(Gui parent) {
		return new GuiElementButton<AQuestManager<T>>(parent, this);
	}

	protected Quest<T> generateQuest(int id, YMLSection section) {
		return new AQuest<T>(id, this, section);
	}
	...
	protected Mission<T> generateMission(int id, Quest<T> quest, YMLSection section) {
		return new AMission<T>(id, quest, section);
	}
	...
	public final void linkRequire(Require<T> require, Quest<T> quest) {
		...
		// database.link("requires_quests", "require_id", require.getID(), "quest_id",
		// quest.getID());
	}
	...
	protected class EditorGui extends PagedMapGui {
		public EditorGui(Player player, Gui previusHolder) {
			super("&9Quest Manager: &6" + getName(), 6, player, previusHolder);
		...
		private class QuestCreate extends AButton {
			...
			private ItemStack item = new ItemBuilder(Material.YELLOW_DYE).setGuiProperty().build();
			...
			public ItemStack getItem() {
				Utils.updateDescription(item, Arrays.asList("&6Click to create a new Quest"),
