...
public class AMission<T extends User<T>> extends AQuestComponentWithCooldown<T> implements Mission<T> {
	...
	/*
	private String getPhaseMessageIsDefaultPath(PhaseChange phase) {
		return phase.toString() + "-message-is-default";
	}*/
	...
	public AMission(int id, Quest<T> quest, YMLSection section) {
		...
		this.parent = quest;
		displayInfo = new MissionDisplayInfo<T>(getConfig().loadSection(Paths.QUESTCOMPONENT_DISPLAY_INFO), this);
	...
	public final ArrayList<String> getRawPhaseMessage(PhaseChange phase) {
		return new ArrayList<String>(rawMessages.get(phase));
	}
	...
	private final ArrayList<String> getDefaultPhaseMessage(PhaseChange phase) {
	...
	protected class AMissionGuiEditor extends AAAGuiEditor {
		...
		private class PhaseMessage extends StringListEditorButton {
			...
			public List<String> getButtonDescription() {
				...
				if (showPhaseMessage(phase)) {
					ArrayList<String> list = new ArrayList<String>(Arrays.asList("&6"+phase.name().toLowerCase()+" Message Editor",
		...
		private class TaskSelector extends GuiElementSelectorButton<Task<T>> {
			...
			@Override
			public List<String> getButtonDescription() {
				return Arrays.asList("&6Click to select a Task");
			}
		...
		private class TaskCreate extends ElementSelectorButton<TaskType<T>> {
			...
			@Override
			public List<String> getButtonDescription() {
				return Arrays.asList("&6Click to create a new Task");
			}
		...
		private class RequireCreate extends ElementSelectorButton<RequireType<T>> {
			...
			@Override
			public List<String> getButtonDescription() {
				return Arrays.asList("&6Click to create a new Require");
			}
		...
		private class StartRewardCreate extends ElementSelectorButton<RewardType<T>> {
			...
			@Override
			public List<String> getButtonDescription() {
				return Arrays.asList("&6Click to create a new StartReward");
			}
		...
		private class CompleteRewardCreate extends ElementSelectorButton<RewardType<T>> {
			...
			@Override
			public List<String> getButtonDescription() {
				return Arrays.asList("&6Click to create a new CompleteReward");
			}
		...
		private class FailRewardCreate extends ElementSelectorButton<RewardType<T>> {
			...
			@Override
			public List<String> getButtonDescription() {
				return Arrays.asList("&6Click to create a new FailReward");
			}
	...
	private final static Material guiFullMaterial(PhaseChange phase) {
		switch (phase) {
		case COMPLETE:
	...
	private final static Material guiGlassMaterial(PhaseChange phase) {
		switch (phase) {
		case COMPLETE:
