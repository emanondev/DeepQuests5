...
import emanondev.deepquests.interfaces.BossBarManager;
...
public class ABossBarManager<T extends User<T>> implements BossBarManager<T> {
	...
	public void onProgress(T user, Task<T> task) {
		...
		if (!map.containsKey(user)) {
			...
			tMap.put(task, new TaskBossBar<T>(user.getTaskData(task)));
			map.put(user, tMap);
		} else if (!map.get(user).containsKey(task)) {
			map.get(user).put(task, new TaskBossBar<T>(user.getTaskData(task)));
		}
