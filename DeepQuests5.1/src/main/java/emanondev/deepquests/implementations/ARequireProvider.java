...
import emanondev.deepquests.interfaces.RequireProvider;
...
public class ARequireProvider<T extends User<T>> implements RequireProvider<T> {
	...
	public Require<T> getInstance(int id, QuestManager<T> manager, YMLSection section) {
		if (manager == null)
			throw new NullPointerException();
		try {
			Require<T> require = getType(section.getString(Paths.TYPE_NAME, null)).getInstance(id, manager, section);
			/*
			 * Quests.get().logDone("Loaded require &e" + id + " &f(Type: &e" +
		...
		return new ErrorRequire<T>(id, manager, section);
	}
