...
abstract class ADisplayInfo<T extends User<T>, E extends HasDisplay<T>> implements DisplayInfo<T> {
	...
	private final EnumMap<DisplayState, Boolean> isDefHidden = new EnumMap<DisplayState, Boolean>(DisplayState.class);
	private final EnumMap<DisplayState, Boolean> isDefItem = new EnumMap<DisplayState, Boolean>(DisplayState.class);
	private final EnumMap<DisplayState, Boolean> isDefDesc = new EnumMap<DisplayState, Boolean>(DisplayState.class);
	private final EnumMap<DisplayState, Boolean> hidden = new EnumMap<DisplayState, Boolean>(DisplayState.class);
	private final EnumMap<DisplayState, ItemStack> item = new EnumMap<DisplayState, ItemStack>(DisplayState.class);
	private final EnumMap<DisplayState, List<String>> desc = new EnumMap<DisplayState, List<String>>(
			DisplayState.class);
	...
	private List<String> getDefaultDescription(DisplayState state) {
		return getParent().getManager().getConfig().loadStringList(
				getBasePath() + "." + state.name().toLowerCase() + ".description", new ArrayList<String>());
	}
	...
	public ArrayList<String> getRawDescription(DisplayState state) {
		return new ArrayList<String>(desc.get(state));
	}
	 ...
	 * @param state
	 * @param item
	 *            - setting as null revert to default
	 */
	@Override
				...
				public HiddenFlagButton(DisplayState state) {
					super(Utils.setDescription(new ItemBuilder(Material.LIME_WOOL).setGuiProperty().build(),
							...
							Utils.setDescription(new ItemBuilder(Material.RED_WOOL).setGuiProperty().build(),
									Arrays.asList("&6This status is &chidden"), DisplaySelector.this.getTargetPlayer(),
									true),
