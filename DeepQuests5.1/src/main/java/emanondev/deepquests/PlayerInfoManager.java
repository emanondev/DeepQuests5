...
public class PlayerInfoManager implements Listener {
	...
	/* TODO removed
	private static final String FILE_NAME = "player_info";
	private static final String TABLE_NAME = "player_info";
	private static final String URL = "jdbc:sqlite:" + Quests.get().getDataFolder() + File.separator + FILE_NAME
			+ ".sqlite.db";
	private final Connection CONN;
	private final PreparedStatement PS_SELECT;
	private final PreparedStatement PS_LOAD;
	private final PreparedStatement PS_INSERT;
	...
	*/
	...
	PlayerInfoManager() throws SQLException {
		/* TODO removed
		CONN = DriverManager.getConnection(URL);
		createTable();

		PS_SELECT = CONN.prepareStatement("SELECT uuid FROM " + TABLE_NAME + " WHERE uuid = ?");
		PS_LOAD = CONN.prepareStatement("SELECT * FROM " + TABLE_NAME + " WHERE uuid = ?");

		StringBuilder sql = new StringBuilder("UPDATE " + TABLE_NAME + " SET");
		for (DisplayState state : DisplayState.values()) {
			sql.append(" quest_" + state.toString().toLowerCase() + " = ? " + ", mission_"
					+ state.toString().toLowerCase() + " =  ? " + ",");
		}
		sql.append(" bartype = ? , barstyle = ? , name = ? WHERE uuid = ?");
		PS_UPDATE = CONN.prepareStatement(sql.toString());

		sql = new StringBuilder("INSERT INTO " + TABLE_NAME + " (uuid,name,");
		StringBuilder sql2 = new StringBuilder(") VALUES (?,?,");
		for (DisplayState state : DisplayState.values()) {
			sql.append("quest_" + state.toString().toLowerCase() + ",mission_" + state.toString().toLowerCase() + ",");
			sql2.append("?,?,");
		}
		sql.append("bartype,barstyle" + sql2.toString() + "?,?);");
		PS_INSERT = CONN.prepareStatement(sql.toString());*/
		loadAll();
	...
	public class PlayerInfoSqlite implements PlayerInfo {
