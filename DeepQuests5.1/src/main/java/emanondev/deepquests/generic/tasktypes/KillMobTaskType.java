...
public class KillMobTaskType<T extends User<T>> extends ATaskType<T> {
	...
	public class KillMobTask extends ATask<T> {
		...
		private EntityData<T, KillMobTask> entityData = null;
		private DropData<T, KillMobTask> dropsData = null;
		private ToolData<T, KillMobTask> toolData = null;

		public KillMobTask(int id, Mission<T> mission, YMLSection section) {
			super(id, mission, KillMobTaskType.this, section);
			dropsData = new DropData<T, KillMobTask>(this, getConfig().loadSection(Paths.TASK_INFO_DROPDATA));
			entityData = new EntityData<T, KillMobTask>(this, getConfig().loadSection(Paths.TASK_INFO_ENTITYDATA));
			toolData = new ToolData<T, KillMobTask>(this, getConfig().loadSection(Paths.TASK_INFO_TOOLDATA));
		}
	...
	public String getDefaultUnstartedDescription(Task<T> task) {
		if (!(task instanceof KillMobTaskType.KillMobTask))
			return null;
		KillMobTask t = (KillMobTask) task;
