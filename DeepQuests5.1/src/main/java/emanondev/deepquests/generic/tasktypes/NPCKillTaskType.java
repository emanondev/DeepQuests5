...
public class NPCKillTaskType<T extends User<T>> extends ATaskType<T> {
	...
	public class NPCKillTask extends ATask<T> {
		...
		private NPCData<T, NPCKillTask> npcData = null;
		private ToolData<T, NPCKillTask> toolData = null;
		private DropData<T, NPCKillTask> dropsData = null;

		public NPCKillTask(int id, Mission<T> mission, YMLSection section) {
			super(id, mission, NPCKillTaskType.this, section);
			npcData = new NPCData<T, NPCKillTask>(this, getConfig().loadSection(Paths.TASK_INFO_NPCDATA));
			toolData = new ToolData<T, NPCKillTask>(this, getConfig().loadSection(Paths.TASK_INFO_TOOLDATA));
			dropsData = new DropData<T, NPCKillTask>(this, getConfig().loadSection(Paths.TASK_INFO_DROPDATA));
		}
	...
	public String getDefaultUnstartedDescription(Task<T> task) {
		if (!(task instanceof NPCKillTaskType.NPCKillTask))
			return null;
		NPCKillTask t = (NPCKillTask) task;
