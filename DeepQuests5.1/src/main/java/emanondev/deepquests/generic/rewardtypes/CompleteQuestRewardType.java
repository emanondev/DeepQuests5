...
public class CompleteQuestRewardType<T extends User<T>> extends ARewardType<T> {
	...
	public List<String> getDescription() {
		return Arrays.asList("&7Complete selected quest for player");
	}
	...
	public class CompleteQuestReward extends AReward<T> {
		private TargetQuestData<T, CompleteQuestReward> questData;
		...
		public CompleteQuestReward(int id, QuestManager<T> manager, YMLSection section) {
			...
			questData = new TargetQuestData<T, CompleteQuestReward>(this,
		...
		public void apply(T qPlayer, int amount) {
			if (amount <= 0)
				return;
			try {
				Quest<T> quest = (Quest<T>) questData.getQuest();
				if (quest == null)
	...
	public String getDefaultFeedback(Reward<T> reward) {
		if (!(reward instanceof CompleteQuestRewardType.CompleteQuestReward))
			return null;
		CompleteQuestReward r = (CompleteQuestReward) reward;
